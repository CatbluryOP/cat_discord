...
RegisterServerEvent('cat_discord:removeFromQueue')
AddEventHandler('cat_discord:removeFromQueue', function()
...
lib.callback.register('cat_discord:getConfig', function(source)
    local cConfig = copyTable(Config)
    cConfig.DiscordInfo = nil 
    return cConfig
end)

lib.callback.register('cat_discord:checkForRole', function(source, role)
    local hasRole = checkRole(source, role)
    return hasRole
end)

lib.callback.register('cat_discord:getRoles', function(source, role)
